...
public class PLPModuleGenerator {
    ...
    public static String GeneratePLPModule(PLP plp) {
        ...
        generator.writeLine("PLP_WAYPOINT_HISTORY_LENGTH = 2");
        generator.newLine();
        ...
        // monitor termination
        generator.writeIndentedBlock(generateTerminationDetectors((AchievePLP) plp, generator.getCurrentTabLevel()));
        //
    ...
    private static String generateTerminationDetectors(AchievePLP plp, int currentTabLevel) {
        PythonWriter generator = new PythonWriter(currentTabLevel);

        generator.writeLine("def detect_success(self):");
        generator.indent();
        generator.writeLine("if "+generateIFcondition(plp.getGoal())+":");
        generator.indent();
        generator.writeLine("return PlpTermination(True, \" Achieved: " +plp.getGoal().toString() + "\")");
        generator.dendent();
        generator.writeLine("else:");
        generator.indent();
        generator.writeLine("return None");
        ...
        generator.indent();
        for (FailureMode fm : plp.getFailureModes()) {
